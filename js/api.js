...
(function(){
    ...
    (function(){
        ...
        function installFallback(){
            ...
            function parseJsonSafe(res) { return res.text().then(text=>{ try{return JSON.parse(text||'{}');}catch(e){ try{ const m = text.match(/(\{[\s\S]*\}|\[[\s\S]*\])/); if (m) return JSON.parse(m[0]); }catch(e2){ safeWarn('api.parseJsonSafe.e2', e2); } if (text.trim().startsWith('<?php')) { return { success:false, error:'Servidor no soporta PHP. Usa un servidor con PHP habilitado (ej. XAMPP, Apache con mod_php).', raw:text }; } return { success:false, error:'Invalid JSON', raw:text }; } }); }
