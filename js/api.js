...
(function(){
    ...
    (function(){
        ...
        function installFallback(){
            ...
            function parseJsonSafe(res) { return res.text().then(text=>{ try{return JSON.parse(text||'{}');}catch(e){ try{ const m = text.match(/(\{[\s\S]*\}|\[[\s\S]*\])/); if (m) return JSON.parse(m[0]); }catch(e2){ safeWarn('api.parseJsonSafe.e2', e2); } if (text.trim().startsWith('<?php') || text.includes('<?php') || text.includes('Fatal error') || text.includes('Parse error') || text.includes('405') || text.includes('Method Not Allowed') || text.includes('REQUEST') || text.includes('array(')) { return { success:false, error:'Error del servidor: No se puede procesar la solicitud. Verifica que el servidor tenga PHP habilitado y configurado correctamente.', raw:text }; } return { success:false, error:'Respuesta inválida del servidor', raw:text }; } }); }
